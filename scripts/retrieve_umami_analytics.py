...
def main():
    ...
    parser.add_argument(
        '--events',
        action='store_true',
        help='Retrieve detailed event data'
    )
    parser.add_argument(
        '--summary',
        action='store_true',
    ...
    if stats:
        ...
        logger.info(f"Stats: {stats.get('pageviews', {}).get('value', 0)} pageviews, "
    ...
    if args.summary:
        ...
        summary_path = args.output / f'summary_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
